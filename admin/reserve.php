	...
	<div class = "container-fluid">	
		<div class = "panel panel-default">
			...
			<div class = "panel-body">
				...
				<a class = "btn btn-info" href = "Enregistrement.php"><span class = "badge"><?php echo $f_ci['total']?></span> Enregistrer</a>
				<a class = "btn btn-warning" href = "checkout.php"><i class = "glyphicon glyphicon-eye-open"></i> Paiement</a>
				<br />
				<br />
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `transaction` NATURAL JOIN `client` NATURAL JOIN `chambre` WHERE `status` = 'attente'") or die(mysqli_error());
							while($fetch = $query->fetch_array()){
						?>
						<tr>
							<td><?php echo $fetch['Prenom']." ".$fetch['nom']?></td>
							<td><?php echo $fetch['numero_tel']?></td>
							<td><?php echo $fetch['chambre_type']?></td>
							<td><strong><?php if($fetch['enregistrement'] <= date("Y-m-d", strtotime("+8 HOURS"))){echo "<label style = 'color:#ff0000;'>".date("M d, Y", strtotime($fetch['enregistrement']))."</label>";}else{echo "<label style = 'color:#00ff00;'>".date("M d, Y", strtotime($fetch['enregistrement']))."</label>";}?></strong></td>
							<td><?php echo $fetch['status']?></td>
							<td><center><a class = "btn btn-success" href = "confirm_reserve.php?transaction_id=<?php echo $fetch['transaction_id']?>"><i class = "glyphicon glyphicon-check"></i> Enregistrement</a> <a class = "btn btn-danger" onclick = "confirmationDelete(); return false;" href = "SuppAttente.php?transaction_id=<?php echo $fetch['transaction_id']?>"><i class = "glyphicon glyphicon-trash"></i> Annuler</a></td>
						</tr>
						<?php
							}
						?>

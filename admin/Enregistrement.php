	...
	<div class = "container-fluid">	
		<div class = "panel panel-default">
			...
			<div class = "panel-body">
				...
				<a class = "btn btn-info disabled"><span class = "badge"><?php echo $f_ci['total']?></span> Enregistrer</a>
				<a class = "btn btn-warning" href = "checkout.php"><i class = "glyphicon glyphicon-eye-open"></i> Payer</a>
				<br />
				<br />
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `transaction` NATURAL JOIN `client` NATURAL JOIN `chambre` WHERE `status` = 'enregistrement'") or die(mysqli_query());
						...
						<tr>
							...
							<td><?php echo "<label style = 'color:#ff0000;'>".date("M d, Y", strtotime($fetch['enregistrement']."+".$fetch['date']."date"))."</label>"?></td>
							<td><?php echo $fetch['status']?></td>
							<td><?php if($fetch['lit_extra'] == "0"){ echo "None";}else{echo $fetch['lit_extra'];}?></td>
							<td><?php echo "".$fetch['facture'].""?></td>
							<td><center><a class = "btn btn-warning" href = "checkout_query.php?transaction_id=<?php echo $fetch['transaction_id']?>" onclick = "confirmationenregistrement(); return false;"><i class = "glyphicon glyphicon-check"></i> Paiement</a></center></td>
						</tr>
						<?php
							}
						?>
...
<script type = "text/javascript">
	function confirmationenregistrement(anchor){
		var conf = confirm("Etes vous surs ?");
		if(conf){
